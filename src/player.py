...
from .db import CACHE, QUEUE, Database, FallbackSong
...
class QueueWatcher(watchdog.events.FileSystemEventHandler):
    ...
    def song_created(self, path) -> None:
        LOG.debug(f"new song added to queue {path.stem}")
...
class Player:
    ...
    def create_fifo(self) -> None:
        ...
        if not FIFO.exists():
            ...
            run(["mkfifo", str(FIFO)], check=True)
    ...
    def play_song(self, path: Path) -> None:
        ...
        LOG.debug(f"pushing {path.stem} to ffmpeg {FIFO.name}")
        run(cmd)

        self.database.add_played(song)
        self.state.clear_current()
        self.state.append_history(current_song.to_history())

    def choose_fallback(self) -> FallbackSong:
        now = datetime.now()

        weights: list[float] = []
        fallback = self.valid_fallback_songs
    ...
    def play_fallback(self) -> None:
        ...
        if not self.fallback:
            self.fallback = Fallback(self.choose_fallback())
        ...
        while True:
            ...
            while self.fallback.offset < duration:
                ...
                cmd = [
                    ...
                    str(this_slice),
                    "-i",
                    str(self.fallback.song.filepath),
                    "-f",
                    "wav",
                    str(FIFO),
                ]
    ...
    def mark_played(self, path: Path) -> None:
        if song := self.database.song_by_name(path.stem):
            LOG.info(f"song {path.stem} finished playback, updating database")
            song.last_played = datetime.now()
            song.play_count += 1
            self.database.update(song)
        else:
            LOG.info(f"song {path.stem} finished playback")
        path.unlink(missing_ok=True)
    ...
    def run(self) -> None:
        ...
        try:
            ...
            if fifo_writer.is_alive():
                self.safe_execute("stop fifo", fifo_writer.join, timeout=10)

            if self.observer:
                self.safe_execute("oserver stop", self.observer.stop)
                self.safe_execute("oserver terminate", self.observer.join, timeout=10)
